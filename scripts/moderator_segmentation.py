...
"""
        ...
        try:
            ...
            for top, info in boundaries.items():
                span = info['end_index'] - info['start_index'] + 1
                transition = info['transition_type']
                announce_idx = info['announcement_index']
                print(f"\n  {top[:60]}...")
                print(f"    → Indices: {info['start_index']}-{info['end_index']} ({span} utterances)")
                print(f"    → Transition: {transition} (announced at: {announce_idx if announce_idx else 'N/A'})")
                print(f"    → Reasoning: {info['reasoning'][:100]}{'...' if len(info['reasoning']) > 100 else ''}")
